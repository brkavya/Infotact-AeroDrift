...
class TestAWSLoader(unittest.TestCase):
    ...
    def test_normalize_ec2_relationships(self):
        ...
        self.assertEqual(normalized["id"], "i-0123456789abcdef0")
        self.assertEqual(len(normalized["relationships"]), 4)

        targets = [rel["target"] for rel in normalized["relationships"]]
